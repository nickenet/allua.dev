...
$help_section = (string)$dumper_help;

$title_cp = 'Управление базой данных - ';
$speedbar = ' | <a href="dumper.php">Управление базой данных</a>';

check_login_cp('0_0','main.php');

require_once 'template/header.php';

table_item_top ("Управление базой данных",'dboption.png');
...
define('DBNUSER', $db_user);
define('DBPREFIX',$prefix_b);
...
class dumper {
	...
	function backup() {
		...
		$buttons = "<span ID=save STYLE='display: none;'>Бекап БД успешно создан в папке ../upload/ </span>&nbsp; &nbsp; <INPUT ID=back TYPE=button VALUE='Закрыть' DISABLED onClick=\"window.close();\">";
		...
		if(!defined('AUTOMODE'))
		{
		  ...
		  $name = $db . '_' . date("Y-m-d_H-i"). '_' . substr( $rand, 0, 5);
...
if( $_GET['mod'] == "dboption" and count( $_REQUEST['ta'] ) ) {
	...
	$db->query ( $query ) or die ('MySQL error db_options.');

	msg_text('80%', $def_admin_message_ok, "Выбранное действие успешно выполнено - $query.");
}
...
$r_tables=$db->query ( "SHOW TABLES" );

$results_tables = $db->numrows ( $r_tables );

	for ( $ii=0;$ii<$results_tables;$ii++ )
	{
	 	$f_tables=$db->fetcharray ($r_tables);
		$tabellen .= "<option value=\"$f_tables[0]\" selected>$f_tables[0]</option>\n";
...
<table width="500" border="0" cellspacing="0" cellpadding="0" align="center">
  <tr>
    ...
    <td background="images/table_sys_02.gif"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="16" align="center"><img src="images/table_sys_vdv.gif" width="8" height="20" /></td>
        <td>Настройка и оптимизация базы данных</td>
        <td width="16" align="right"><img src="images/table_sys_zn.gif" width="16" height="16" border="0" /></td>
      </tr>
    </table></td>
    <td width="5"><img src="images/table_sys_03.gif" width="5" height="26" /></td>
...
<form action="dumper.php?mod=dboption" method="post">
<select style="width:240px;" size="7" name="ta[]" multiple="multiple">
<? echo $tabellen; ?> 
</select><br /><br />
<div align="left" style="padding-left:10px;">
<input style="border:0px" type="radio" name="whattodo" checked="checked" value="optimize" /><b>Оптимизация базы данных</b><br />
<span class="vclass">Вы можете произвести оптимизацию базы данных, тем самым будет сэкономлено немного места на диске, а также ускорена работа базы данных. Рекомендуется использовать данную функцию минимум один раз в месяц.</span><br /><br />
<input style="border:0px" type="radio" name="whattodo" value="repair" /><b>Ремонт базы данных</b><br />
<span class="vclass">При неожиданной остановке MySQL сервера, во время выполнения каких-либо действий, может произойти повреждение структуры таблиц, использование этой функции поможет решить вам эту проблему.</span><br /><br />
<input type="submit" id="rest" class="bbcodes" value="Выполнить действие" /><br /><br />
</div>
</form>
