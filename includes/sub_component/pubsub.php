...

{
	...
	if ( $iii%2 == 0 ) $color = $def_form_back_color;
	...
	$template->replace("shortstory", $f['shortstory']);
		...
		$parts_whv = array();
                preg_match('#wvr(\d+)_hvr(\d+)#', $template_sub_pub, $parts_whv);
	...
	if ($f['video']!='') { 
		...
		$video_on="<img src=\"$def_mainlocation/template/$def_template/images/video_on.gif\" alt=\"$def_video_on\" align=\"absmiddle\" border=\"0\">"; 
                ...
                $vf->link_video=$f['video'];
                $template->replace($replace_video_tags, $vf->showVideo('video'));

	} else $template->replace($replace_video_tags, "");
	...
	$template->replace("hits", "$show_info");
		...
		$days_result = round( $f[period] - ( ( $days ) / ( 60 * 60 * 24 ) ) );
                ...
                if ($f['period']==0) $days_result = $def_period_offers;
