...
if( ! defined( 'ISB' )) {
	...
	$template_cimg->load($template_view_img_category);
        ...
        if ($def_rewrite == "YES") $link_img_category=$def_mainlocation.'/'.$f['selector'].'-gallery'.$def_rewrite_split.rewrite($f['category']).'/'; else $link_img_category=$def_mainlocation.'/allimg.php?category='.$f['selector'];
